...
open class FanRefreshControl: UIRefreshControl {
    ...
    fileprivate func updateTextColor(color:UIColor?){
        if (color != nil) {
            ...
            currentAttribut.addAttributes([NSForegroundColorAttributeName:color!], range: NSRange(location: 0, length: currentAttribut.string.lengthOfBytes(using: .utf8)))
            ...
            for (key,value) in self.fan_stateTitles {
                ...
                attribut.addAttributes([NSForegroundColorAttributeName:color!], range: NSRange(location: 0, length: attribut.string.lengthOfBytes(using: .utf8)))
